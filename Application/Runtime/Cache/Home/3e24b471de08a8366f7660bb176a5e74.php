    ...
    <div id="home_page_roll" style="float:left; overflow:hidden; height:145px; width:560px; margin-top:10px; margin-left:130px;">
        <div id="home_page_roll_demo">
            <div id="home_page_roll_img_1">
                <?php
 $con=new mysqli('localhost','sq_capcccomcn','capcccomcn','capcc'); $result=$con->query("SELECT * FROM news_active where Level = 4 ORDER BY NewsID DESC LIMIT 18"); $row=$result->fetch_array(); $n=1; while($row) { echo "<span id=\"small_img_".$n."\">"; echo "<a id=\"small_a_img_".$n."\" href=\"news/active/?newsid=".$row['NewsID']."\">"; echo "<img id=\"small_real_img_".$n."\" src=\""; echo str_replace("D:/wamp/server/apache/htdocs/capcc/","/capcc0624/Public",str_replace("\\","/",$row['Picture'])); echo "\" width=\"110\" height=\"145\"/>"; echo "</a>"; echo "</span>"; echo "\n"; $n++; $row=$result->fetch_array(); } ?>
            </div>
